...
라즈베리 파이 제품을 고르기 위해 몇 가지 스펙을 결정해야 한다.
...
데이터를 저장하는 장치로는 라즈베리파이에서 지원하는 SD카드. 운영체제 설치까지 고려해서 32GB 는 되어야 할 것 같다.  

SD카드 구매 가이드는 [요 링크](https://makeutil.tistory.com/254)를 참고할 예정이다. 
...
이번에 선물 받은 하기비스 미니 모니터를 연결할 예정이다. 

[키보드]

이 또한 집에 있는 키크론 키보드를 연결할 생각.  

입출력 장치는 큰 걱정이 없다. 어차피 맥북으로 SSH 원격 접속할 예정이기 때문이다.  
